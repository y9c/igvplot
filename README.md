...
| **Read modes** | `color_by`: `strand` · `firstOfPairStrand` · `pairOrientation` · `readgroup` · `mapq` · `insert`/`tlen` · `unexpectedPair` · `proper` · `mate` · `basemod` · `none`. `group_by` clusters reads vertically; `sort_by`: `start` · `strand` · `mapq` · `insert` · `mate_start` · `name`. |
